...
ASSET_JSON = ROOT / "assets" / "onset_stress_b_oblique_shoe" / "onset_stress_b_oblique_shoe_model.json"
OUTPUT_DIR = ROOT / "data" / "outputs"
DOCS_DIR = ROOT / "docs"
FIG_DIR = DOCS_DIR / "figures"
REPORT_PATH = DOCS_DIR / "onset_stress_b_oblique_shoe_ablation_report.html"
...
def ensure_assets():
    subprocess.run([sys.executable, str(ROOT / "scripts" / "generate_onset_stress_b_oblique_shoe_assets.py")], check=True)
...
def shoe_reference(params, times):
    ...
    half_width = params["shoe_half_width"]
    half_height = params["shoe_half_height"]
    radius = params["shoe_corner_radius"]
    ...
    for t in times:
        ...
        if top is None:
            ...
            env_pos = cy + cam_radius + top
            flat_limit = half_width - radius
            dtop_dtheta = 0.0
            ax = abs(dx)
            if ax > flat_limit and ax <= half_width:
                sign = 1.0 if dx >= 0.0 else -1.0
                local = ax - flat_limit
                denom = math.sqrt(max(radius * radius - local * local, 1.0e-16))
                dtop_ddx = -local / denom * sign
                dtop_dtheta = dtop_ddx * (ecc * math.sin(theta))
            env_vel = (ecc * math.cos(theta) + dtop_dtheta) * omega
...
def run_case(name: str, params, extra_env: dict[str, str], output_name: str):
    ...
    env = {
        ...
        "SPCC_ONSET_B_CLUSTER_RADIUS": "0.006",
        "SPCC_ONSET_B_LOCAL_SCAN_RADIUS": "0.012",
    ...
    cmd = "cd {} && {} ./_build/project/baseline_onset_stress_b_nsc --cam-mesh {} --follower-mesh {} --contact-algorithm sdf_2nd --output {} --dt {} --T {} --speed {} --spring-k {} --spring-c {} --spring-rest-length {} --spring-anchor-y {} --spring-anchor-x {} --follower-x {} --follower-y {}".format(
        ...
        to_wsl_path(ROOT / "assets" / "onset_stress_b_oblique_shoe" / "models" / "onset_cam.obj"),
        to_wsl_path(ROOT / "assets" / "onset_stress_b_oblique_shoe" / "models" / "shoe_follower.obj"),
    ...
    ref_rows, onset_time = shoe_reference(params, [row["t"] for row in rows])
    return {
        "name": name,
        "runtime": runtime,
        "rows": rows,
...
def make_plot(results, onset_time):
    ...
    plt.title("Oblique Flat-Nose Follower: Mechanism Ablation")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    out = FIG_DIR / "onset_stress_b_oblique_shoe_ablation_velocity.png"
...
<head>
  <meta charset="utf-8" />
  <title>Oblique Flat-Nose Follower Ablation</title>
  <style>
    body {{
...
<body>
  <h1>Oblique Flat-Nose Follower: Mechanism Ablation</h1>
  <p>
    This stronger stress variant replaces the circular roller with a smooth rounded-rectangle shoe. The flatter nose
    creates a wider early-contact footprint, which is intended to make positive-gap onset candidates more competitive.
  </p>
  <table>
    <thead>
      <tr>
  ...
  <div class="fig">
    ...
    <img src="{plot_path.relative_to(DOCS_DIR).as_posix()}" alt="Oblique flat-nose follower ablation" />
  </div>
...
def main():
    ...
    cases = [
        ("default", {}, "onset_stress_b_oblique_shoe_default.csv"),
        (
            "no_gate",
            {
            ...
            "onset_stress_b_oblique_shoe_no_gate.csv",
        ),
        (
            "no_fit",
            {
            ...
            "onset_stress_b_oblique_shoe_no_fit.csv",
        ),
    ]
    results = [run_case(name, params, env, output) for name, env, output in cases]
