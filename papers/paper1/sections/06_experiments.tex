...
\begin{figure}[htbp]
    ...
    \caption{静态误差与简单动力学误差的同步时序对比。灰色区间表示 dense 与 reduced 均存在活动接触。该图用于观察瞬时 $\varepsilon_F$、$\varepsilon_M$、$\varepsilon_{\mathrm{CoP}}$、$\varepsilon_g$ 与线冲量误差、角冲量误差、角速度误差和能量漂移差在同一时间轴上的耦合关系。}
...
\begin{figure}[htbp]
    ...
    \caption{静态误差到简单动力学误差的散点相关图。每个点对应一个时间步的活动接触状态，图中同时给出 Pearson 相关系数，用于辅助判断瞬时静态误差与动力学偏差之间的统计联系。}
...
\begin{figure}[htbp]
    ...
    \caption{时序连续性与简单动力学稳定性的对应关系。当前原型验证表明，reduced support 的连续性退化会直接放大角速度与冲量层面的动态误差，因此动力学精度应被理解为“瞬时静态等效 + 时序连续性”的联合结果。}
